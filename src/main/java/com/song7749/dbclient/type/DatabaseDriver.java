...
public enum DatabaseDriver {
	...
	ORACLE(
			...
			"select 1 from dual",
			// table list
			"SELECT T1.TABLE_NAME TABLE_NAME,T2.COMMENTS TABLE_COMMENT FROM ALL_TABLES T1, DBA_TAB_COMMENTS T2 WHERE T2.TABLE_NAME(+) = T1.TABLE_NAME and  T1.OWNER=upper('{schemaOwner}') order by TABLE_NAME asc",
			// field list
			...
			// view source
			"SELECT uv.VIEW_NAME as NAME, uv.TEXT FROM ALL_VIEWS uv JOIN ALL_OBJECTS uo ON (uv.VIEW_NAME=uo.OBJECT_NAME AND uo.object_type = 'VIEW') WHERE uv.OWNER=upper('{schemaOwner}')  AND uv.VIEW_NAME=upper('{name}')",
