...
function program11(depth0,data) {
  ...
  buffer += escapeExpression(stack1)+ "'" ; 
  // song7749 fix
  if(depth0["format"] === "date-time"){
	  buffer += " onclick='addDate(this);'";
  }
  buffer += "/>\n			";
...
function program15(depth0,data) {
  ...
  buffer += "' placeholder='(required)' type='text' value=''/>\n			";
  return buffer;
  }
    ...
    SwaggerUi.prototype.buildUrl = function(base, url) {
      ...
      if (url.indexOf("/") === 0) {
        ...
        if (base.indexOf("/", base.length - 1) !== -1) {
            log("API JSON URL 2: "+base + url);
        	return base + url;
        }
        log("API JSON URL 3: "+base + url);
        return base + "/" + url;
      }
    };
