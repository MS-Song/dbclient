...
public class SrDataRequestModifyBeforeConfirmDto extends AbstractDto {
	...
	public SrDataRequestModifyBeforeConfirmDto(@NotNull Long id, @Size(min = 8, max = 200) String subject,
			@Size(min = 8, max = 50000) String runSql, YN enableYN, Integer downloadLimit,
			DownloadLimitType downloadLimitType, Date downloadStartDate, Date downloadEndDate, @NotNull Long databaseId,
			List<String> conditionWhereSql, List<String> conditionWhereSqlKey, List<String> conditionName,
			List<String> conditionKey, List<DataType> conditionType, List<String> conditionValue,
			List<YN> conditionRequired, List<Long> srDataAllowMemberIds, Long memberId) {
		this.id = id;
