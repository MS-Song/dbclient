...
public class BaseExceptionResolver extends SimpleMappingExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		ResponseResult responseResult = new ResponseResult(responseCode, message);
		...
		String ext = ".html";
		if(request.getRequestURI().lastIndexOf(ext)==request.getRequestURI().length()-ext.length()){
	 ...
	 */
	private String convertExceptionMessage(HttpServletRequest request,Object handler, Exception ex){
				...
				for(Object parameterName : request.getParameterMap().keySet()){
					...
					for(MethodParameter parameter : ((HandlerMethod)handler).getMethodParameters()){
						...
						if(parameter.hasParameterAnnotation(RequestParam.class)){
							...
							if(parameterName.toString().equals(parameter.getParameterAnnotation(RequestParam.class).value())){
								// 파싱을 해서 처리하려고 했으나 오버헤드가 너무 크다.
								// exception 에서 전달한 값과 같은 값을 찾아서 값이 여러개이면, 의심된다고 보내고, 하나면 찾아서 보낸다.
								String paramValue = request.getParameter(parameterName.toString());
								String ExceptionValue= ((TypeMismatchException)ex).getValue().toString();
								// 파라메터의 값이 존재하고, 익셉션에서 찾은 값이 동일한 경우에만 파라메터에 대한 내용을 보여준다.
								if( paramValue != null
										&& ExceptionValue != null
									...
									sb.append(" , type : ");
									sb.append(parameter.getNestedParameterType().getSimpleName());
									sb.append(" , value : ");
